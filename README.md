...
本项目为基于 NestJS + Docker + 多中间件（MySQL、Redis、MongoDB、Consul 等）的分布式系统，支持前后端分离、插件扩展、监控链路追踪等能力。
...
| MySQL        | 3306   | 数据库连接端口        |
| Redis        | 6379   | 缓存服务              |
| MongoDB      | 27017  | 数据存储（插件用）    |
| Consul       | 8500   | 服务发现注册中心      |
| RabbitMQ     | 15672  | 消息队列管理界面      |
| Prometheus   | 9090   | 系统监控              |
| Grafana      | 3000   | 可视化展示（与 Prometheus 配合） |
| Jaeger       | 16686  | 分布式链路追踪 UI     |
...
- 请确保 `.env` 文件与 `docker-compose.yml` 在同一目录
- `backend` 和 `frontend` 可以独立开发调试
- 前端建议生产环境使用 `npm run build` 并用 Nginx 托管 `dist/`

---

## 🧠 作者提示

如你在部署过程中遇到问题，可联系开发团队，或使用以下命令排查日志：
```bash
